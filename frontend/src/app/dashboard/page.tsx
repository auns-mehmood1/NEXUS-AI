  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: '2rem' }}>
        {[
          { icon: 'Req', label: 'Total Requests', value: usage.totalRequests.toLocaleString(), sub: 'All time' },
          { icon: 'Lat', label: 'Avg Latency', value: `${usage.avgLatency}s`, sub: 'Last 24h' },
          { icon: 'Cost', label: 'Total Cost', value: `$${usage.totalCost.toFixed(2)}`, sub: 'This month' },
          { icon: 'AI', label: 'Models Used', value: usage.topModels.length.toString(), sub: 'Unique models' },
        ].map((card) => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
        {[
          { href: '/chat', icon: 'CH', title: 'Open Chat Hub', desc: 'Start or continue conversations' },
          { href: '/marketplace', icon: 'MK', title: 'Browse Models', desc: 'Discover 400+ AI models' },
          { href: '/agents', icon: 'AG', title: 'Build Agent', desc: 'Create your first AI agent' },
          { href: '/dashboard/history', icon: 'HI', title: 'View History', desc: 'See all past conversations' },
        ].map((link) => (
