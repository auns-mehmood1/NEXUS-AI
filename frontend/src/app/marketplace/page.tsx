...
const PROVIDERS = ['All', 'OpenAI', 'Anthropic', 'Google', 'Meta', 'DeepSeek', 'Alibaba', 'xAI', 'Mistral', 'Cohere', 'Microsoft'];
...
export default function MarketplacePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [typeFilter, providerFilter, ratingMin, maxPrice, searchText, sortBy]);
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--white)', borderBottom: '1px solid var(--border)', padding: '1.5rem 2rem' }}>
        ...
        <p style={{ color: 'var(--text2)', fontSize: '0.9rem' }}>Discover and compare {MODELS_DATA.length}+ AI models from leading providers</p>
      </div>
        ...
        <aside style={{ width: 240, flexShrink: 0, borderRight: '1px solid var(--border)', padding: '1.25rem', background: 'var(--white)' }}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            {(['rating', 'price', 'reviews'] as const).map(s => (
              <label key={s} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.3rem 0', cursor: 'pointer', fontSize: '0.82rem', color: 'var(--text2)' }}>
                <input type="radio" checked={sortBy === s} onChange={() => setSortBy(s)} style={{ accentColor: 'var(--accent)' }} />
                {s === 'rating' ? 'Top Rated' : s === 'price' ? 'Lowest Price' : 'Most Reviews'}
              </label>
            ))}
          </div>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            {PROVIDERS.map(p => (
              <label key={p} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0.3rem 0', cursor: 'pointer', fontSize: '0.82rem', color: providerFilter === p ? 'var(--accent)' : 'var(--text2)' }}>
                <input type="radio" checked={providerFilter === p} onChange={() => setProviderFilter(p)} style={{ accentColor: 'var(--accent)' }} />
                {p}
              </label>
            ))}
          </div>
        ...
        <div style={{ flex: 1, padding: '1.25rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
            {filtered.slice(0, 100).map(m => (
              <MarketplaceCard key={m.id} model={m} onView={() => setSelectedModel(m)} onUseInChat={() => router.push(`/chat?model=${m.id}`)} />
            ))}
          </div>
          {filtered.length > 100 && (
  ...
  return (
    <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '1.1rem', boxShadow: 'var(--shadow)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: '0.6rem' }}>
        ...
        <div style={{ fontSize: '0.78rem', color: 'var(--text2)', display: 'flex', alignItems: 'center', gap: 2 }}>⭐ {model.rating}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: '0.75rem' }}>
        {model.tags.slice(0, 3).map(t => <span key={t} style={{ background: 'var(--bg2)', borderRadius: '2rem', padding: '0.12rem 0.5rem', fontSize: '0.68rem', color: 'var(--text2)' }}>{t}</span>)}
      </div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onClose}>
      <div style={{ background: 'var(--white)', borderRadius: 20, maxWidth: 560, width: '100%', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 5, marginTop: 5, flexWrap: 'wrap' }}>
              {model.badge && <span className={model.badgeClass}>{model.badge}</span>}
              {model.tags.slice(0, 4).map(t => <span key={t} style={{ background: 'var(--bg2)', borderRadius: '2rem', padding: '0.12rem 0.5rem', fontSize: '0.7rem', color: 'var(--text2)' }}>{t}</span>)}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', padding: '0 1.5rem' }}>
          {tabs.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: '0.75rem 1rem', border: 'none', background: 'none', fontSize: '0.82rem', fontWeight: tab === t ? 600 : 400, color: tab === t ? 'var(--accent)' : 'var(--text2)', borderBottom: tab === t ? '2px solid var(--accent)' : '2px solid transparent', cursor: 'pointer', fontFamily: 'inherit' }}>{t}</button>
          ))}
        </div>
        {/* Content */}
          ...
          {tab === 'Overview' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {[{k:'Rating',v:`⭐ ${model.rating}`},{k:'Reviews',v:model.reviews.toLocaleString()},{k:'Context',v:model.context},{k:'Pricing',v:model.price},{k:'Provider',v:model.org},{k:'Types',v:model.types.join(', ')}].map(s => (
                  <div key={s.k} style={{ textAlign: 'center', background: 'var(--bg)', borderRadius: 8, padding: '0.75rem', border: '1px solid var(--border)' }}>
                    <div style={{ fontWeight: 700, fontSize: '0.88rem' }}>{s.v}</div>
                    <div style={{ fontSize: '0.65rem', color: 'var(--text3)', textTransform: 'uppercase' }}>{s.k}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {tab === 'Reviews' && (
            <div>
              {[
                ...
                <div key={i} style={{ background: 'var(--bg)', borderRadius: 10, padding: '1rem', marginBottom: 10, border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontWeight: 600, fontSize: '0.85rem' }}>{r.user}</span>
                    <span style={{ fontSize: '0.82rem' }}>{'⭐'.repeat(r.rating)}</span>
                  </div>
                  <p style={{ fontSize: '0.82rem', color: 'var(--text2)' }}>{r.text}</p>
                </div>
              ))}
            </div>
          )}
